...
Não implemente uma solução pontual para EncounterICU.
...
## Novo recurso a ingerir: EncounterICU

Arquivo:

* `./data/MimicEncounterICU.ndjson.gz`
...
* nas amostras observadas, **não foi identificado `serviceProvider.reference`**
* portanto, **não assuma relacionamento com `organization` para EncounterICU**, a menos que ele realmente exista no arquivo
* a implementação deve ser robusta para tratar esse campo como opcional, mas não deve inventar mapeamento sem evidência

---

## Diretriz de modelagem para EncounterICU

Mantenha a filosofia de modelagem **enxuta e pragmática**.

Como `EncounterICU` representa uma variação / especialização de encounter de UTI, a modelagem deve preservar isso de forma clara.

### Criar tabela principal `encounter_icu`
...
### Regras de simplificação para `encounter_icu`
...
* **Não carregar**:
  ...
  * `type[*].coding[*]`
  * `class.system`
  * qualquer campo não essencial nesta fase

### Relacionamento com location

Como `EncounterICU` possui `location[*]`, e um ICU encounter pode estar associado a uma ou mais locations, você deve preservar esse relacionamento.

Crie uma tabela auxiliar enxuta:

### Criar tabela `encounter_icu_location`
...
Cada item de `location[*]` deve gerar um registro nessa tabela.

### Justificativa esperada

A modelagem deve deixar claro que:

* `encounter_icu` é uma tabela especializada para internações/atendimentos ICU
* `encounter` continua sendo a tabela principal de encounters gerais
* `encounter_icu.encounter_id` faz o vínculo com o encounter “pai” por meio de `partOf.reference`
...
Não crie FK de `encounter_icu` para `organization` sem evidência real no arquivo.
Se posteriormente houver esse campo em outros arquivos, isso poderá ser evoluído depois.
...
## Regras de extração para EncounterICU
...
Cada item da lista `location[*]` deve gerar um registro em `encounter_icu_location`.
...
Atualize os diagramas para refletir a modelagem já existente e incluir `encounter_icu`.
...
* referência inválida em `subject`
* referência inválida em `location`
* garantia de que apenas as colunas simplificadas são retornadas para `encounter_icu`
* garantia de que a tabela derivada `encounter_icu_location` é montada corretamente
...
A execução principal deve ingerir os seis arquivos na ordem definida.
...
* explicação dos relacionamentos:
  ...
  * `encounter_icu -> patient`
  * `encounter_icu_location -> encounter_icu`
  * `encounter_icu_location -> location`
...
* suporte a ingestão de `EncounterICU`
* atualização da ordem da pipeline
* criação da tabela `encounter_icu`
* criação da tabela `encounter_icu_location`
* criação das FKs de `encounter_icu`
* atualização do `TABLE_RELATIONSHIPS.md`
...
6. explicação breve da nova modelagem com `encounter_icu`
