...
11. `./data/MimicCondition.ndjson.gz`

Esta entrega dá início à **quinta fase de ingestão de arquivos**.

A ordem de importação deve ser tratada de forma consistente com a pipeline já existente, preservando os recursos anteriores e adicionando `Condition`.
...
A inclusão de `Condition` deve seguir o mesmo padrão arquitetural já adotado.
...
## Novo recurso a ingerir: Condition
...
Portanto, `Condition` **deve possuir relacionamento relacional com `Patient` e `Encounter`**.
...
* não crie relacionamento com outras tabelas sem evidência explícita no arquivo

---

## Diretriz de modelagem para Condition

Mantenha a filosofia de modelagem **enxuta e pragmática**.

### Criar tabela principal `condition`
...
### Regras de simplificação para `condition`
...
Não crie FKs extras para `Condition` sem evidência explícita no arquivo.
...
## Regras de extração para Condition
...
* usar o **primeiro valor não vazio e válido encontrado** em `code.coding[*]`
* usar o **primeiro valor não vazio e válido encontrado** em `category[*].coding[*]`

### Observação

Nesta fase, manter o modelo simples.
Não criar tabela separada para múltiplos codings nem múltiplas categorias.
...
* `src/ingestion/transformers/condition_transformer.py`
...
* `src/ingestion/loaders/condition_loader.py`
...
* `src/pipelines/ingest_condition.py`
...
* `tests/unit/test_condition_transformer.py`
...
Atualize os diagramas para refletir a modelagem já existente e incluir `condition`.
...
A execução principal deve ingerir os onze arquivos na ordem definida.
...
* separação entre primeira, segunda, terceira, quarta e quinta fase de ingestão
* pré-requisitos
* configuração do `.env`
...
* explicação de que `condition` entra nesta fase com relacionamento para `patient` e `encounter`
...
* início da quinta fase de ingestão
* suporte a ingestão de `Condition`
* atualização da ordem da pipeline
* criação da tabela `condition`
...
6. explicação breve da nova modelagem com `condition`
