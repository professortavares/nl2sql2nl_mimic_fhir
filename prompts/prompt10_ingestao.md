...
9. `./data/MimicMedicationRequest.ndjson.gz`

Esta entrega dá seguimento à **terceira fase de ingestão de arquivos**.

A ordem de importação deve ser tratada de forma consistente com a pipeline já existente, preservando os recursos anteriores e adicionando `MedicationRequest`.
...
A inclusão de `MedicationRequest` deve seguir o mesmo padrão arquitetural já adotado.
...
## Novo recurso a ingerir: MedicationRequest

Arquivo:

* `./data/MimicMedicationRequest.ndjson.gz`
...
* `medicationReference.reference = "Medication/<medication_id>"`

Portanto, `MedicationRequest` **deve possuir relacionamento relacional com `Patient`, `Encounter` e `Medication`**.

---

## Diretriz de modelagem para MedicationRequest

Mantenha a filosofia de modelagem **enxuta e pragmática**.

### Criar tabela principal `medication_request`
...
### Regras de simplificação para `medication_request`
...
* **Não criar tabelas auxiliares** para:
  ...
  * `medication_request_meta_profile`
...
Não crie FKs extras para `MedicationRequest` sem evidência explícita no arquivo.
...
## Regras de extração para MedicationRequest

### Tabela `medication_request`
...
* `medicationReference.reference` -> `medication_id`
* `intent`
* `status`
* `authoredOn` -> `authored_on`
* `identifier[*].value` -> `identifier`
* `dispenseRequest.validityPeriod.start` -> `validity_start`
* `dispenseRequest.validityPeriod.end` -> `validity_end`
...
* `dosageInstruction[*].doseAndRate[*].doseQuantity.value` -> `dose_value`
* `dosageInstruction[*].doseAndRate[*].doseQuantity.unit` -> `dose_unit`
...
Mesmo que futuramente o recurso permita mais de uma instrução de dosagem, nesta fase a modelagem deve permanecer simplificada e consolidar apenas o primeiro conjunto válido.
...
Atualize os diagramas para refletir a modelagem já existente e incluir `medication_request`.
...
Mostrar `medication` como dimensão base de medicamentos.
...
A execução principal deve ingerir os nove arquivos na ordem definida.
...
* explicação de que `medication` entra como dimensão base
* explicação de que `medication_mix` se relaciona com `medication`
* explicação de que `medication_request` se relaciona com `patient`, `encounter` e `medication`
...
* suporte a ingestão de `MedicationRequest`
* atualização da ordem da pipeline
* criação da tabela `medication_request`
* criação das FKs para `patient`, `encounter` e `medication`
* atualização do `TABLE_RELATIONSHIPS.md`
...
6. explicação breve da nova modelagem com `medication_request`
