...
14. `./data/MimicProcedureED.ndjson.gz`
...
Não criar FK direta para `procedure`, `encounter_ed` ou outras tabelas sem referência explícita no arquivo.

## Modelagem

Criar tabela `procedure_ed` com:
...
- `procedure_ed_code_coding`
- `procedure_ed_meta_profile`
...
- `src/ingestion/transformers/procedure_ed_transformer.py`
- `src/ingestion/loaders/procedure_ed_loader.py`
- `src/pipelines/ingest_procedure_ed.py`
...
- `tests/unit/test_procedure_ed_transformer.py`
...
### procedureED com patient e encounter
...
Criar testes para `procedure_ed_transformer`.
...
* início da ingestão de `ProcedureED`
* arquivo processado
...
* descrição da tabela `procedure_ed`
...
* suporte a `MimicProcedureED`
* criação da tabela `procedure_ed`
