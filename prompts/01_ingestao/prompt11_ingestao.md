...
10. `./data/MimicSpecimen.ndjson.gz`

Esta entrega dá início à **quarta fase de ingestão de arquivos**.

A ordem de importação deve ser tratada de forma consistente com a pipeline já existente, preservando os recursos anteriores e adicionando `Specimen`.
...
A inclusão de `Specimen` deve seguir o mesmo padrão arquitetural já adotado.
...
## Novo recurso a ingerir: Specimen

Arquivo:

* `./data/MimicSpecimen.ndjson.gz`
...
Foi identificado relacionamento explícito com:

* `subject.reference = "Patient/<patient_id>"`

Portanto, `Specimen` **deve possuir relacionamento relacional com `Patient`**.

### Observação importante

Nas amostras observadas, **não foram identificadas referências explícitas** para:
...
* **não invente foreign keys adicionais**
* não crie relacionamento com `encounter` ou outras tabelas sem evidência explícita no arquivo

---

## Diretriz de modelagem para Specimen

Mantenha a filosofia de modelagem **enxuta e pragmática**.

### Criar tabela principal `specimen`
...
### Regras de simplificação para `specimen`
...
* **Não criar tabelas auxiliares** para:
  ...
  * `specimen_identifier`
  * `specimen_type_coding`
  * `specimen_meta_profile`

### Regra obrigatória para listas
...
### Nova foreign key obrigatória

* `specimen.patient_id -> patient.id`

### Importante

Não crie FKs extras para `Specimen` sem evidência explícita no arquivo.
...
## Regras de extração para Specimen
...
* `type.coding[*].code` -> `specimen_type_code`
* `type.coding[*].system` -> `specimen_type_system`
* `type.coding[*].display` -> `specimen_type_display`
* `collection.collectedDateTime` -> `collected_at`
* `identifier[*].value` -> `identifier`

### Regras obrigatórias

* usar o **primeiro valor não vazio e válido encontrado** em `type.coding[*]`
* usar o **primeiro valor não vazio e válido encontrado** em `identifier[*]`

### Observação

Nesta fase, manter o modelo simples.
Não criar tabela separada para múltiplos codings nem múltiplos identificadores.
...
* `src/ingestion/transformers/medication_request_transformer.py`
* `src/ingestion/transformers/specimen_transformer.py`
...
* `src/ingestion/loaders/medication_request_loader.py`
* `src/ingestion/loaders/specimen_loader.py`
...
* `src/pipelines/ingest_medication_request.py`
* `src/pipelines/ingest_specimen.py`
...
* `tests/unit/test_medication_request_transformer.py`
* `tests/unit/test_specimen_transformer.py`
...
Atualize os diagramas para refletir a modelagem já existente e incluir `specimen`.
...
Mostrar `medication` como dimensão base.
...
#### 9. medication_request com patient, encounter e medication
...
### 11) Transformer de MedicationRequest
...
* registro válido com:
  ...
  * `dosageInstruction`
* registro sem `identifier`
* registro sem `dispenseRequest`
* registro sem `dosageInstruction`
* registro sem `encounter`
* referência inválida em `subject`
* referência inválida em `encounter`
* referência inválida em `medicationReference`

### 12) Transformer de Specimen
...
A execução principal deve ingerir os dez arquivos na ordem definida.
...
* separação entre primeira, segunda, terceira e quarta fase de ingestão
* pré-requisitos
* configuração do `.env`
...
* explicação de que `specimen` entra nesta fase com relacionamento para `patient`
...
* início da quarta fase de ingestão
* suporte a ingestão de `Specimen`
* atualização da ordem da pipeline
* criação da tabela `specimen`
* criação da FK para `patient`
* atualização do `TABLE_RELATIONSHIPS.md`
...
6. explicação breve da nova modelagem com `specimen`
