"""
Pipeline de ingestão do recurso FHIR Specimen.
"""
...
class SpecimenIngestionPipeline:
    """
    Coordena leitura, transformação e persistência de Specimen.
    """

    def __init__(self, settings: ProjectSettings, loader: SpecimenLoader) -> None:
        """
        Inicializa o pipeline.
        """

        self._settings = settings.specimen
        self._common_settings = settings.common
        self._loader = loader
        self._reader = NdjsonGzipReader(self._settings.input_path)
        self._transformer = SpecimenTransformer()
    ...
    def ingest(self, connection: Connection) -> ResourceIngestionSummary:
        """
        Executa a ingestão de Specimen usando uma conexão já aberta.
        """
