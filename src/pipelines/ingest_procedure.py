"""
Pipeline de ingestão do recurso FHIR Procedure.
"""
...
class ProcedureIngestionPipeline:
    """
    Coordena leitura, transformação e persistência de Procedure.
    """

    def __init__(self, settings: ProjectSettings, loader: ProcedureLoader) -> None:
        """
        Inicializa o pipeline.
        """

        self._settings = settings.procedure
        self._common_settings = settings.common
        self._loader = loader
        self._reader = NdjsonGzipReader(self._settings.input_path)
        self._transformer = ProcedureTransformer()
    ...
    def ingest(self, connection: Connection) -> ResourceIngestionSummary:
        """
        Executa a ingestão de Procedure usando uma conexão já aberta.
        """
