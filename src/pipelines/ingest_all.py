...
from src.ingestion.loaders.observation_datetimeevents_loader import ObservationDatetimeeventsLoader
from src.ingestion.loaders.observation_outputevents_loader import ObservationOutputeventsLoader
...
from src.ingestion.loaders.medication_administration_loader import MedicationAdministrationLoader
...
from src.pipelines.ingest_observation_datetimeevents import (
    ObservationDatetimeeventsIngestionPipeline,
)
from src.pipelines.ingest_observation_outputevents import ObservationOutputeventsIngestionPipeline
from src.pipelines.ingest_observation_ed import ObservationEDIngestionPipeline
from src.pipelines.ingest_observation_vital_signs_ed import (
...
from src.pipelines.ingest_medication_administration import MedicationAdministrationIngestionPipeline
from src.pipelines.ingest_medication_administration_icu import MedicationAdministrationICUIngestionPipeline
from src.pipelines.ingest_medication_statement_ed import MedicationStatementEDIngestionPipeline
...
class IngestAllPipeline:
    ...
    def __init__(self, settings: ProjectSettings) -> None:
        ...
        metadata, tables = build_project_metadata(
            ...
            settings.observation_datetimeevents.table_name,
            settings.observation_outputevents.table_name,
            settings.observation_ed.table_name,
            settings.observation_vital_signs_ed.table_name,
            ...
            settings.medication_administration.table_name,
            settings.medication_administration_icu.table_name,
            settings.medication_statement_ed.table_name,
        )
        ...
        self._observation_datetimeevents_loader = ObservationDatetimeeventsLoader(
            tables=tables.observation_datetimeevents,
            patient_tables=tables.patient,
            encounter_tables=tables.encounter,
        )
        self._observation_outputevents_loader = ObservationOutputeventsLoader(
            tables=tables.observation_outputevents,
            patient_tables=tables.patient,
            encounter_tables=tables.encounter,
        )
        ...
        self._pipelines = {
            ...
            "observation_datetimeevents": ObservationDatetimeeventsIngestionPipeline(
                settings=settings,
                loader=self._observation_datetimeevents_loader,
            ),
            "observation_outputevents": ObservationOutputeventsIngestionPipeline(
                settings=settings,
                loader=self._observation_outputevents_loader,
            ),
            "observation_ed": ObservationEDIngestionPipeline(settings=settings, loader=self._observation_ed_loader),
            "observation_vital_signs_ed": ObservationVitalSignsEDIngestionPipeline(
                settings=settings,
                loader=self._observation_vital_signs_ed_loader,
            ),
            "medication_dispense": MedicationDispenseIngestionPipeline(
                settings=settings,
            ...
            "medication_administration": MedicationAdministrationIngestionPipeline(
                settings=settings,
                loader=self._medication_administration_loader,
            ),
            "medication_administration_icu": MedicationAdministrationICUIngestionPipeline(
                settings=settings,
                loader=self._medication_administration_icu_loader,
            ),
            "medication_statement_ed": MedicationStatementEDIngestionPipeline(
                settings=settings,
                loader=self._medication_statement_ed_loader,
            ),
        }
