"""
Pipeline de ingestão do recurso FHIR ProcedureED.
"""
...
class ProcedureEDIngestionPipeline:
    """
    Coordena leitura, transformação e persistência de ProcedureED.
    """

    def __init__(self, settings: ProjectSettings, loader: ProcedureEDLoader) -> None:
        """
        Inicializa o pipeline.
        """

        self._settings = settings.procedure_ed
        self._common_settings = settings.common
        self._loader = loader
        self._reader = NdjsonGzipReader(self._settings.input_path)
        self._transformer = ProcedureEDTransformer()
    ...
    def ingest(self, connection: Connection) -> ResourceIngestionSummary:
        """
        Executa a ingestão de ProcedureED usando uma conexão já aberta.
        """
