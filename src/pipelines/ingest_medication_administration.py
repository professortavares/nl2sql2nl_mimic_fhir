"""
Pipeline de ingestão do recurso FHIR MedicationAdministration.
"""
...
class MedicationAdministrationIngestionPipeline:
    """
    Coordena leitura, transformação e persistência de MedicationAdministration.
    """

    def __init__(self, settings: ProjectSettings, loader: MedicationAdministrationLoader) -> None:
        """
        Inicializa o pipeline.
        """

        self._settings = settings.medication_administration
        self._common_settings = settings.common
        self._loader = loader
        self._reader = NdjsonGzipReader(self._settings.input_path)
        self._transformer = MedicationAdministrationTransformer()
    ...
    def ingest(self, connection: Connection) -> ResourceIngestionSummary:
        """
        Executa a ingestão de MedicationAdministration usando uma conexão já aberta.
        """
