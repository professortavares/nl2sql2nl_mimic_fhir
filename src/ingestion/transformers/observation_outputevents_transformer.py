"""
Transformação de recursos FHIR `Observation` da camada output.
"""
...
class ObservationOutputeventsTransformationError(ValueError):
    """Indica que uma ObservationOutputevents não passou na validação mínima."""


class ObservationOutputeventsTransformer:
    """
    ...
    def transform(self, resource: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if resource.get("resourceType") != _RESOURCE_TYPE:
            raise ObservationOutputeventsTransformationError(
                f"resourceType inválido para ObservationOutputevents: {resource.get('resourceType')!r}"
            )

        observation_id = first_non_empty_text(resource.get("id"))
        if observation_id is None:
            raise ObservationOutputeventsTransformationError("O campo obrigatório 'id' está ausente ou vazio.")
    ...
    def _extract_reference_id(
        ...
        if reference is None:
            raise ObservationOutputeventsTransformationError(f"O campo '{field_name}.reference' é obrigatório.")
        try:
            return parse_fhir_reference(reference, expected_type)
        except (TypeError, ValueError, FhirReferenceParseError) as exc:
            raise ObservationOutputeventsTransformationError(str(exc)) from exc
    ...
    def _extract_coding_value(self, container: Any, key: str) -> str | None:
        ...
        if not isinstance(container, Mapping):
            raise ObservationOutputeventsTransformationError("O campo 'code' deve ser um objeto FHIR quando presente.")
