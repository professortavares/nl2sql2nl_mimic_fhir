"""
Transformação de recursos FHIR `Condition` para a modelagem simplificada.
"""
...
class ConditionTransformationError(ValueError):
    """Indica que um recurso Condition não passou na validação mínima."""
...
class ConditionTransformer:
    ...
    em `code.coding[*]` e `category[*].coding[*]`.
    """
    ...
    def transform(self, resource: Mapping[str, Any]) -> dict[str, Any]:
        ...
        -------
        dict[str, Any]
            Registro pronto para persistência na tabela `condition`.
        ...
        if resource.get("resourceType") != _RESOURCE_TYPE:
            raise ConditionTransformationError(
                f"resourceType inválido para Condition: {resource.get('resourceType')!r}"
            )

        condition_id = first_non_empty_text(resource.get("id"))
        if condition_id is None:
            raise ConditionTransformationError("O campo obrigatório 'id' está ausente ou vazio.")
    ...
    def _extract_reference_id(self, reference_container: Any, expected_type: str) -> str | None:
        ...
        if reference is None:
            raise ConditionTransformationError("A referência FHIR está incompleta.")
        try:
            return parse_fhir_reference(reference, expected_type)
        except (TypeError, ValueError, FhirReferenceParseError) as exc:
            raise ConditionTransformationError(str(exc)) from exc
    ...
    def _extract_coding_value(self, container: Any, key: str) -> str | None:
        ...
        if not isinstance(container, Mapping):
            raise ConditionTransformationError("O campo 'code' deve ser um objeto FHIR quando presente.")
    ...
    def _extract_category_value(self, categories: Any, key: str) -> str | None:
        ...
        if not isinstance(categories, list):
            raise ConditionTransformationError("O campo 'category' deve ser uma lista quando presente.")
