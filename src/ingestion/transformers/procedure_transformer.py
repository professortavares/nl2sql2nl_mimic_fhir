"""
Transformação de recursos FHIR `Procedure` para a modelagem simplificada.
"""
...
class ProcedureTransformationError(ValueError):
    """Indica que um recurso Procedure não passou na validação mínima."""


class ProcedureTransformer:
    """
    Converte recursos FHIR Procedure em um dicionário relacional enxuto.
    ...
    def transform(self, resource: Mapping[str, Any]) -> dict[str, Any]:
        ...
        -------
        dict[str, Any]
            Registro pronto para persistência na tabela `procedure`.
        ...
        if resource.get("resourceType") != _RESOURCE_TYPE:
            raise ProcedureTransformationError(
                f"resourceType inválido para Procedure: {resource.get('resourceType')!r}"
            )

        procedure_id = first_non_empty_text(resource.get("id"))
        if procedure_id is None:
            raise ProcedureTransformationError("O campo obrigatório 'id' está ausente ou vazio.")
    ...
    def _extract_reference_id(
        ...
        if reference is None:
            raise ProcedureTransformationError(f"O campo '{field_name}.reference' é obrigatório.")
        try:
            return parse_fhir_reference(reference, expected_type)
        except (TypeError, ValueError, FhirReferenceParseError) as exc:
            raise ProcedureTransformationError(str(exc)) from exc
    ...
    def _extract_coding_value(self, container: Any, key: str) -> str | None:
        ...
        if not isinstance(container, Mapping):
            raise ProcedureTransformationError("O campo 'code' deve ser um objeto FHIR quando presente.")
