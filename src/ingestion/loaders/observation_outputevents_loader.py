"""
Persistência das linhas normalizadas de ObservationOutputevents no PostgreSQL.
"""
...
from src.db.schema import EncounterTables, ObservationOutputeventsTables, PatientTables

LOGGER = logging.getLogger(__name__)


@dataclass(slots=True, frozen=True)
class ObservationOutputeventsBatchInsertCounts:
    """Resumo das linhas inseridas em um lote de ObservationOutputevents."""
    ...
    def table_counts(self) -> dict[str, int]:
        ...
        return {"observation_outputevents": self.primary_rows}


class ObservationOutputeventsLoader:
    """
    Persiste batches de ObservationOutputevents na tabela simplificada.
    """

    def __init__(
        self,
        tables: ObservationOutputeventsTables,
    ...
    @property
    def tables(self) -> ObservationOutputeventsTables:
        """
    ...
    def insert_batch(
    ...
    ) -> ObservationOutputeventsBatchInsertCounts:
        """
        Persiste um lote de observações output transformadas.
        """

        if not batch:
            return ObservationOutputeventsBatchInsertCounts(0)
        ...
        connection.execute(insert(self._tables.observation_outputevents), normalized_batch)
        return ObservationOutputeventsBatchInsertCounts(
    ...
    def _nullify_orphan_references(
        ...
        *,
        ...
        for reference_id, count in orphan_counts.items():
            LOGGER.warning(
                "ObservationOutputevents com %s órfão não encontrado: %s=%s linhas=%s",
                warning_label,
                warning_label,
                reference_id,
                count,
