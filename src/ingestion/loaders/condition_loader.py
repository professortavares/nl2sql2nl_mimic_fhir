"""
Persistência das linhas normalizadas de Condition no PostgreSQL.
"""
...
from src.db.schema import ConditionTables, EncounterTables, PatientTables

LOGGER = logging.getLogger(__name__)


@dataclass(slots=True, frozen=True)
class ConditionBatchInsertCounts:
    """Resumo das linhas inseridas em um lote de Condition."""
    ...
    def table_counts(self) -> dict[str, int]:
        ...
        return {"condition": self.primary_rows}


class ConditionLoader:
    """
    Persiste batches de Condition na tabela simplificada.
    """
    ...
    @property
    def tables(self) -> ConditionTables:
        """
    ...
    def insert_batch(
    ...
    ) -> ConditionBatchInsertCounts:
        """
        Persiste um lote de conditions transformadas.
        """

        if not batch:
            return ConditionBatchInsertCounts(0)
        ...
        connection.execute(insert(self._tables.condition), normalized_batch)
        return ConditionBatchInsertCounts(
    ...
    def _nullify_orphan_references(
        ...
        *,
        ...
        for reference_id, count in orphan_counts.items():
            LOGGER.warning(
                "Condition com %s órfão não encontrado: %s=%s linhas=%s",
                warning_label,
                warning_label,
                reference_id,
                count,
