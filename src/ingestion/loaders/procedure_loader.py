"""
Persistência das linhas normalizadas de Procedure no PostgreSQL.
"""
...
class ProcedureBatchInsertCounts:
    """Resumo das linhas inseridas em um lote de Procedure."""
    ...
    def table_counts(self) -> dict[str, int]:
        ...
        return {"procedure": self.primary_rows}


class ProcedureLoader:
    """
    Persiste batches de Procedure na tabela simplificada.
    """
    ...
    @property
    def tables(self) -> ProcedureTables:
        """
    ...
    def insert_batch(
    ...
    ) -> ProcedureBatchInsertCounts:
        """
        Persiste um lote de procedures transformadas.
        """

        if not batch:
            return ProcedureBatchInsertCounts(0)
        ...
        connection.execute(insert(self._tables.procedure), normalized_batch)
        return ProcedureBatchInsertCounts(
    ...
    def _nullify_orphan_references(
        ...
        *,
        ...
        for reference_id, count in orphan_counts.items():
            LOGGER.warning(
                "Procedure com %s órfão não encontrado: %s=%s linhas=%s",
                warning_label,
                warning_label,
                reference_id,
                count,
