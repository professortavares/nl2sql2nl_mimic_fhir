"""
Persistência das linhas normalizadas de MedicationStatementED no PostgreSQL.
"""
...
from src.db.schema import EncounterTables, MedicationStatementEDTables, PatientTables
...
class MedicationStatementEDBatchInsertCounts:
    """Resumo das linhas inseridas em um lote de MedicationStatementED."""
    ...
    def table_counts(self) -> dict[str, int]:
        ...
        return {"medication_statement_ed": self.primary_rows}


class MedicationStatementEDLoader:
    """
    Persiste batches de MedicationStatementED na tabela simplificada.
    """

    def __init__(
        self,
        tables: MedicationStatementEDTables,
    ...
    @property
    def tables(self) -> MedicationStatementEDTables:
        """
    ...
    def insert_batch(
    ...
    ) -> MedicationStatementEDBatchInsertCounts:
        """
        Persiste um lote de medication statements ED transformadas.
        """

        if not batch:
            return MedicationStatementEDBatchInsertCounts(0)
        ...
        connection.execute(insert(self._tables.medication_statement_ed), normalized_batch)
        return MedicationStatementEDBatchInsertCounts(
    ...
    def _nullify_orphan_references(
        ...
        *,
        ...
        for reference_id, count in orphan_counts.items():
            LOGGER.warning(
                "MedicationStatementED com %s órfão não encontrado: %s=%s linhas=%s",
                warning_label,
                warning_label,
                reference_id,
                count,
