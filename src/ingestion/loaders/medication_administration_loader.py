"""
Persistência das linhas normalizadas de MedicationAdministration no PostgreSQL.
"""
...
from src.db.schema import EncounterTables, MedicationAdministrationTables, MedicationRequestTables, PatientTables

LOGGER = logging.getLogger(__name__)


@dataclass(slots=True, frozen=True)
class MedicationAdministrationBatchInsertCounts:
    """Resumo das linhas inseridas em um lote de MedicationAdministration."""

    primary_rows: int
    orphan_patient_references: int = 0
    orphan_encounter_references: int = 0
    orphan_medication_request_references: int = 0
    ...
    def table_counts(self) -> dict[str, int]:
        ...
        return {"medication_administration": self.primary_rows}


class MedicationAdministrationLoader:
    """
    Persiste batches de MedicationAdministration na tabela simplificada.
    """

    def __init__(
        self,
        tables: MedicationAdministrationTables,
        patient_tables: PatientTables,
        encounter_tables: EncounterTables,
        medication_request_tables: MedicationRequestTables,
    ) -> None:
        """
        ...
        self._medication_request_tables = medication_request_tables

    @property
    def tables(self) -> MedicationAdministrationTables:
        """
    ...
    def insert_batch(
    ...
    ) -> MedicationAdministrationBatchInsertCounts:
        """
        Persiste um lote de medication administrations transformadas.
        """

        if not batch:
            return MedicationAdministrationBatchInsertCounts(0)
        ...
        valid_medication_request_ids = self._fetch_existing_ids(
            connection=connection,
            table=self._medication_request_tables.medication_request,
        ...
        orphan_medication_request_references = self._nullify_orphan_references(
            batch=normalized_batch,
            reference_key="medication_request_id",
            valid_ids=valid_medication_request_ids,
            warning_label="medication_request_id",
        )

        connection.execute(insert(self._tables.medication_administration), normalized_batch)
        return MedicationAdministrationBatchInsertCounts(
            ...
            orphan_medication_request_references=orphan_medication_request_references,
        )
    ...
    def _nullify_orphan_references(
        ...
        *,
        ...
        for reference_id, count in orphan_counts.items():
            LOGGER.warning(
                "MedicationAdministration com %s órfão não encontrado: %s=%s linhas=%s",
                warning_label,
                warning_label,
                reference_id,
                count,
