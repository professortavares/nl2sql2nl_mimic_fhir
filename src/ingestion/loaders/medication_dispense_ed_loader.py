"""
Persistência das linhas normalizadas de MedicationDispenseED no PostgreSQL.
"""
...
from src.db.schema import EncounterTables, MedicationDispenseEDTables, PatientTables

LOGGER = logging.getLogger(__name__)


@dataclass(slots=True, frozen=True)
class MedicationDispenseEDBatchInsertCounts:
    """Resumo das linhas inseridas em um lote de MedicationDispenseED."""
    ...
    def table_counts(self) -> dict[str, int]:
        ...
        return {"medication_dispense_ed": self.primary_rows}


class MedicationDispenseEDLoader:
    """
    Persiste batches de MedicationDispenseED na tabela simplificada.
    """

    def __init__(
        self,
        tables: MedicationDispenseEDTables,
    ...
    @property
    def tables(self) -> MedicationDispenseEDTables:
        """
    ...
    def insert_batch(
    ...
    ) -> MedicationDispenseEDBatchInsertCounts:
        """
        Persiste um lote de medication dispenses ED transformadas.
        """

        if not batch:
            return MedicationDispenseEDBatchInsertCounts(0)
        ...
        connection.execute(insert(self._tables.medication_dispense_ed), normalized_batch)
        return MedicationDispenseEDBatchInsertCounts(
    ...
    def _nullify_orphan_references(
        ...
        *,
        ...
        for reference_id, count in orphan_counts.items():
            LOGGER.warning(
                "MedicationDispenseED com %s órfão não encontrado: %s=%s linhas=%s",
                warning_label,
                warning_label,
                reference_id,
                count,
