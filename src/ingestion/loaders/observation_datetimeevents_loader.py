"""
Persistência das linhas normalizadas de ObservationDatetimeevents no PostgreSQL.
"""
...
from src.db.schema import EncounterTables, ObservationDatetimeeventsTables, PatientTables

LOGGER = logging.getLogger(__name__)


@dataclass(slots=True, frozen=True)
class ObservationDatetimeeventsBatchInsertCounts:
    """Resumo das linhas inseridas em um lote de ObservationDatetimeevents."""
    ...
    def table_counts(self) -> dict[str, int]:
        ...
        return {"observation_datetimeevents": self.primary_rows}


class ObservationDatetimeeventsLoader:
    """
    Persiste batches de ObservationDatetimeevents na tabela simplificada.
    """

    def __init__(
        self,
        tables: ObservationDatetimeeventsTables,
    ...
    @property
    def tables(self) -> ObservationDatetimeeventsTables:
        """
    ...
    def insert_batch(
    ...
    ) -> ObservationDatetimeeventsBatchInsertCounts:
        """
        Persiste um lote de observações datadas transformadas.
        """

        if not batch:
            return ObservationDatetimeeventsBatchInsertCounts(0)
        ...
        connection.execute(insert(self._tables.observation_datetimeevents), normalized_batch)
        return ObservationDatetimeeventsBatchInsertCounts(
    ...
    def _nullify_orphan_references(
        ...
        *,
        ...
        for reference_id, count in orphan_counts.items():
            LOGGER.warning(
                "ObservationDatetimeevents com %s órfão não encontrado: %s=%s linhas=%s",
                warning_label,
                warning_label,
                reference_id,
                count,
