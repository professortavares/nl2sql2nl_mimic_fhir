"""
Persistência das linhas normalizadas de MedicationDispense no PostgreSQL.
"""
...
from src.db.schema import EncounterTables, MedicationDispenseTables, MedicationRequestTables, PatientTables

LOGGER = logging.getLogger(__name__)


@dataclass(slots=True, frozen=True)
class MedicationDispenseBatchInsertCounts:
    """Resumo das linhas inseridas em um lote de MedicationDispense."""
    ...
    def table_counts(self) -> dict[str, int]:
        ...
        return {"medication_dispense": self.primary_rows}


class MedicationDispenseLoader:
    """
    Persiste batches de MedicationDispense na tabela simplificada.
    """

    def __init__(
        self,
        tables: MedicationDispenseTables,
    ...
    @property
    def tables(self) -> MedicationDispenseTables:
        """
    ...
    def insert_batch(
    ...
    ) -> MedicationDispenseBatchInsertCounts:
        """
        Persiste um lote de medication dispenses transformadas.
        """

        if not batch:
            return MedicationDispenseBatchInsertCounts(0)
        ...
        connection.execute(insert(self._tables.medication_dispense), normalized_batch)
        return MedicationDispenseBatchInsertCounts(
    ...
    def _nullify_orphan_references(
        ...
        *,
        ...
        for reference_id, count in orphan_counts.items():
            LOGGER.warning(
                "MedicationDispense com %s órfão não encontrado: %s=%s linhas=%s",
                warning_label,
                warning_label,
                reference_id,
                count,
