"""
Persistência das linhas normalizadas de ObservationMicroTest no PostgreSQL.
"""
...
from src.db.schema import EncounterTables, ObservationMicroTestTables, PatientTables, SpecimenTables

LOGGER = logging.getLogger(__name__)


@dataclass(slots=True, frozen=True)
class ObservationMicroTestBatchInsertCounts:
    """Resumo das linhas inseridas em um lote de ObservationMicroTest."""

    primary_rows: int
    orphan_patient_references: int = 0
    orphan_specimen_references: int = 0
    ...
    def table_counts(self) -> dict[str, int]:
        ...
        return {"observation_micro_test": self.primary_rows}


class ObservationMicroTestLoader:
    """
    Persiste batches de ObservationMicroTest na tabela simplificada.
    """

    def __init__(
        self,
        tables: ObservationMicroTestTables,
        patient_tables: PatientTables,
        specimen_tables: SpecimenTables,
        ...
        self._specimen_tables = specimen_tables
        self._encounter_tables = encounter_tables

    @property
    def tables(self) -> ObservationMicroTestTables:
        """
    ...
    def insert_batch(
    ...
    ) -> ObservationMicroTestBatchInsertCounts:
        """
        Persiste um lote de observações microbiológicas transformadas.
        """

        if not batch:
            return ObservationMicroTestBatchInsertCounts(0)
        ...
        valid_specimen_ids = self._fetch_existing_ids(
            connection=connection,
            table=self._specimen_tables.specimen,
        ...
        orphan_specimen_references = self._nullify_orphan_references(
            batch=normalized_batch,
            reference_key="specimen_id",
            valid_ids=valid_specimen_ids,
            warning_label="specimen_id",
        )
        ...
        connection.execute(insert(self._tables.observation_micro_test), normalized_batch)
        return ObservationMicroTestBatchInsertCounts(
            primary_rows=len(normalized_batch),
            orphan_patient_references=orphan_patient_references,
            orphan_specimen_references=orphan_specimen_references,
    ...
    def _nullify_orphan_references(
        ...
        *,
        ...
        for reference_id, count in orphan_counts.items():
            LOGGER.warning(
                "ObservationMicroTest com %s órfão não encontrado: %s=%s linhas=%s",
                warning_label,
                warning_label,
                reference_id,
                count,
