"""
Persistência das linhas normalizadas de ConditionED no PostgreSQL.
"""
...
class ConditionEDBatchInsertCounts:
    """Resumo das linhas inseridas em um lote de ConditionED."""
    ...
    def table_counts(self) -> dict[str, int]:
        ...
        return {"condition_ed": self.primary_rows}


class ConditionEDLoader:
    """
    Persiste batches de ConditionED na tabela simplificada.
    """

    def __init__(
        self,
        tables: ConditionEDTables,
    ...
    @property
    def tables(self) -> ConditionEDTables:
        """
    ...
    def insert_batch(
    ...
    ) -> ConditionEDBatchInsertCounts:
        """
        Persiste um lote de conditions ED transformadas.
        """

        if not batch:
            return ConditionEDBatchInsertCounts(0)
        ...
        connection.execute(insert(self._tables.condition_ed), normalized_batch)
        return ConditionEDBatchInsertCounts(
    ...
    def _nullify_orphan_references(
        ...
        *,
        ...
        for reference_id, count in orphan_counts.items():
            LOGGER.warning(
                "ConditionED com %s órfão não encontrado: %s=%s linhas=%s",
                warning_label,
                warning_label,
                reference_id,
                count,
