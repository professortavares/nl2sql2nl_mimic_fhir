"""
Persistência das linhas normalizadas de ObservationChartevents no PostgreSQL.
"""
...
from src.db.schema import ObservationCharteventsTables, EncounterTables, PatientTables

LOGGER = logging.getLogger(__name__)


@dataclass(slots=True, frozen=True)
class ObservationCharteventsBatchInsertCounts:
    """Resumo das linhas inseridas em um lote de ObservationChartevents."""
    ...
    def table_counts(self) -> dict[str, int]:
        ...
        return {"observation_chartevents": self.primary_rows}


class ObservationCharteventsLoader:
    """
    Persiste batches de ObservationChartevents na tabela simplificada.
    """

    def __init__(
        self,
        tables: ObservationCharteventsTables,
    ...
    @property
    def tables(self) -> ObservationCharteventsTables:
        """
    ...
    def insert_batch(
    ...
    ) -> ObservationCharteventsBatchInsertCounts:
        """
        Persiste um lote de observações charted transformadas.
        """

        if not batch:
            return ObservationCharteventsBatchInsertCounts(0)
        ...
        connection.execute(insert(self._tables.observation_chartevents), normalized_batch)
        return ObservationCharteventsBatchInsertCounts(
    ...
    def _nullify_orphan_references(
        ...
        *,
        ...
        for reference_id, count in orphan_counts.items():
            LOGGER.warning(
                "ObservationChartevents com %s órfão não encontrado: %s=%s linhas=%s",
                warning_label,
                warning_label,
                reference_id,
                count,
