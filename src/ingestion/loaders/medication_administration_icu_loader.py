"""
Persistência das linhas normalizadas de MedicationAdministrationICU no PostgreSQL.
"""
...
class MedicationAdministrationICUBatchInsertCounts:
    """Resumo das linhas inseridas em um lote de MedicationAdministrationICU."""
    ...
    def table_counts(self) -> dict[str, int]:
        ...
        return {"medication_administration_icu": self.primary_rows}


class MedicationAdministrationICULoader:
    """
    Persiste batches de MedicationAdministrationICU na tabela simplificada.
    """
    ...
    def insert_batch(
    ...
    ) -> MedicationAdministrationICUBatchInsertCounts:
        """
        Persiste um lote de medication administrations ICU transformadas.
        """

        if not batch:
            return MedicationAdministrationICUBatchInsertCounts(0)
        ...
        connection.execute(insert(self._tables.medication_administration_icu), normalized_batch)
        return MedicationAdministrationICUBatchInsertCounts(
    ...
    def _nullify_orphan_references(
        ...
        *,
        ...
        for reference_id, count in orphan_counts.items():
            LOGGER.warning(
                "MedicationAdministrationICU com %s órfão não encontrado: %s=%s linhas=%s",
                warning_label,
                warning_label,
                reference_id,
                count,
